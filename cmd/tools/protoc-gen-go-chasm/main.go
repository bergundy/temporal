...
import (
	...
	"go.temporal.io/server/api/routing/v1"
...
func (p *Plugin) Run(plugin *protogen.Plugin) error {
	...
	for _, file := range plugin.Files {
		...
		// create the file
		generatedFile := p.Plugin.NewGeneratedFile(file.GeneratedFilenamePrefix+generatedFilenameExtension, file.GoImportPath)
...
func genAssignShard(m *protogen.Method) (string, error) {
	opts := routingOptions(m)
	if opts == nil {
		return "", fmt.Errorf("No routing directive specified on %s", m.Desc.FullName())
	}
	if opts.ExecutionId == "" {
		return "", fmt.Errorf("ExecutionID directive empty on %s", m.Desc.FullName())
	}
...
func routingOptions(m *protogen.Method) *routing.RoutingOptions {
	opts, _ := proto.GetExtension(m.Desc.Options(), routing.E_Routing).(*routing.RoutingOptions)
	return opts
}
