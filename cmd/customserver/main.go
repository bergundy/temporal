// customserver is a near-copy of cmd/server/main.go that wires in the
// nexusauthz custom Authorizer and ClaimMapper. The point of this binary is
// to demonstrate Nexus identity propagation without modifying any upstream
// file, so it stays rebase-friendly.
//
// The only differences from the upstream `temporal-server` binary are:
//
//   - The default Authorizer is replaced by nexusauthz.NewAuthorizer, which
//     reads a YAML policy file at startup (path from
//     NEXUS_AUTHZ_POLICY_FILE, defaulting to ./policy.yaml).
//   - The default ClaimMapper is replaced by nexusauthz.NewClaimMapper, which
//     wraps the upstream DefaultJWTClaimMapper.
//
// Everything else (config loading, service selection, logger setup, audience
// getter) mirrors upstream verbatim.
package main
...
import (
	...
	"go.temporal.io/server/cmd/customserver/nexusauthz"
...
const (
	// nexusAuthzPolicyFileEnv is the environment variable that points to the
	// YAML policy file consumed by the custom Nexus authorizer.
	nexusAuthzPolicyFileEnv = "NEXUS_AUTHZ_POLICY_FILE"
	// defaultPolicyFile is used when nexusAuthzPolicyFileEnv is unset.
	defaultPolicyFile = "./policy.yaml"
)

// main entry point for the custom temporal server.
func main() {
	app := buildCLI()
	_ = app.Run(os.Args)
}

// buildCLI mirrors cmd/server/main.go.buildCLI, replacing only the authorizer
// and claim mapper instantiation.
func buildCLI() *cli.App {
	app := cli.NewApp()
	app.Name = "customserver"
	app.Usage = "Temporal server with custom Nexus identity-propagation authorizer"
	...
	app.Commands = []*cli.Command{
		...
		{
			Name:      "start",
			Usage:     "Start Temporal server with the custom Nexus authorizer",
			ArgsUsage: " ",
			Flags: []cli.Flag{
				&cli.StringFlag{
					Name:    "services",
			...
			Action: func(c *cli.Context) error {
				...
				_ = c.Bool("allow-no-auth") // not used: custom authorizer is always present
				...
				// Load the Nexus authz policy file. Path is configurable via
				// env var; falls back to ./policy.yaml in the working dir.
				policyPath := os.Getenv(nexusAuthzPolicyFileEnv)
				if policyPath == "" {
					policyPath = defaultPolicyFile
				}
				policy, err := nexusauthz.LoadPolicy(policyPath)
				if err != nil {
					return cli.Exit(fmt.Sprintf("Unable to load Nexus authz policy from %q: %v", policyPath, err), 1)
				}
				logger.Info("Loaded Nexus authz policy.",
					tag.NewStringTag("path", policyPath))

				authorizer := nexusauthz.NewAuthorizer(policy, logger)
				...
				s, err := temporal.NewServer(
					...
					temporal.WithClaimMapper(func(cfg *config.Config) authorization.ClaimMapper {
						return nexusauthz.NewClaimMapper(cfg, logger)
					}),
